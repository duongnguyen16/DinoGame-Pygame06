...
pixels = pygame.surfarray.pixels2d(game_display)
pixels ^= 2 ** 32 - 1
del pixels

# mac os optimize
pygame.display.set_caption("Duong Nguyen Xuan - PYGAME07 (Dino Project Final / Optimized for Mac)")
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, high_score, game_seed):
        ...
        self.max_y_height = float(999999999)

        # SECOND
        self.jump_time = 1.2
        self.t = self.jump_time * 2
    ...
    def update(self):
        ...
        if self.active_timer:
            self.timer = self.current - self.start_current
        else:
            self.timer = 0
        ...
        if not self.game_over:
            ...
            if key[pygame.K_w] or key[pygame.K_SPACE] or key[pygame.K_UP]:
                if not self.state == "jump" and not self.state == "start" and self.wait == 0:
            ...
            if key[pygame.K_s] or key[pygame.K_LSHIFT] or key[pygame.K_DOWN]:
                if not self.state == "duck":
                    self.state = "duck"
                    # print("Duck!")

        if self.state == "jump":
            dddt = 165*math.cos(((2*math.pi) / self.t) * (self.timer / 1000))
            self.rect.y = round (87 + abs(165*math.cos(((2*math.pi) / self.t) * (self.timer / 1000))))
            print(f"163*cos(((2 * pi) / {self.t}) * ({self.timer} / 1000)) {dddt}")
            # print(f"Timer:{self.active_timer} (CURRENT: {self.current}) 87 + abs(163*math.cos((({2 * math.pi}) / {self.t})*{self.timer/1000})) = {jump_locate}")
            if dddt < -164:
                self.rect.y = 250
                self.state = "idle"
                self.wait = 0
        ...
        # print(f"Rect.y {self.rect.y} - Vel: {self.vel} - State: {self.state} - id: {self.id} - timer: {self.timer}")
...
while True:
    ...
    if player.state == "die" and 376 <= player_cursor_x <= 424 and 200 <= player_cursor_y <= 243 and pygame.mouse.get_pressed() == (
    1, 0, 0):
        reset = True
